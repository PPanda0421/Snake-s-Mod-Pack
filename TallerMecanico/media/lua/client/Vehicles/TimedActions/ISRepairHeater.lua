...
ISRepairHeater = ISBaseTimedAction:derive("ISRepairHeater")

function ISRepairHeater:isValid()
--	return self.vehicle:isInArea(self.part:getArea(), self.character)
	return true;
end

function ISRepairHeater:waitToStart()
	self.character:faceThisObject(self.vehicle)
	return self.character:shouldBeTurning()
end

function ISRepairHeater:update()
	self.character:faceThisObject(self.vehicle)
...
function ISRepairHeater:start()
	self.item:setJobType(getText("IGUI_RepairHeater"))
	self:setActionAnim("VehicleWorkOnMid")
end

function ISRepairHeater:stop()
	self.item:setJobDelta(0)
	ISBaseTimedAction.stop(self)
end

function ISRepairHeater:perform()
	ISBaseTimedAction.perform(self)
	self.item:setJobDelta(0)
	local skill = self.character:getPerkLevel(Perks.Mechanics) - 6;
	local numberOfParts = self.character:getInventory():getNumberOfItem("ElectronicsScrap", false, true);
	...
	sendClientCommand(self.character, 'vehicletmmod', 'repairHeater', args)
--[[
...
function ISRepairHeater:new(character, part, item, time)
	local o = {}
	setmetatable(o, self)
	self.__index = self
	...
	o.item = item
	o.maxTime = time
	o.jobType = getText("IGUI_RepairHeater")
	return o
end
