...
function ISInventoryPane:doContextualDblClick(item)
	...
	if item:getType() == "InventoryNote" then
        --if playerObj:getMoodles():getMoodleLevel(MoodleType.FoodEaten) < 3 or playerObj:getNutrition():getCalories() < 1000 then
		if InventoryNoteDef then
			LDIMenu.doModData(item);
		end
		LDIMenu.ReadInvNote_OnCreate(item, self.player)
        --end
	end
end
...
function DoModSystem(itm)
	...
	if LeGourmetMod ~= nil then
		...
		if itm:getType() == "Ghillie_Trousers" or itm:getType() == "Ghillie_Top" then
			itm:setTooltip(getText("Tooltip_GhillieHunt"));
		end
