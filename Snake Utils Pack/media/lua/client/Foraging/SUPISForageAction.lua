...
-- function ISForageAction.FindPickAxe(chr, Def)
	-- local playerInv = chr:getInventory();
	-- local handitem = chr:getPrimaryHandItem();
	-- local pickaxe = false;
	-- for i=1, #Def.weaponHands do
		-- if playerInv:containsTypeEvalRecurse(Def.weaponHands[i], predicateNotBroken) then
			-- pickaxe = true;
		-- end
		-- if pickaxe then 
			-- if (not handItem or handItem:getType() ~= Def.weaponHands[i]) then
				-- local item = playerInv:getFirstTypeEvalRecurse(Def.weaponHands[i], predicateNotBroken)
				-- ISWorldObjectContextMenu.equip(chr, chr:getPrimaryHandItem(), item, true)
				-- break;
			-- end
		-- end
	-- end
