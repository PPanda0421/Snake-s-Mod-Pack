...
AMOpenGuideAction = ISBaseTimedAction:derive("AMOpenGuideAction");

function AMOpenGuideAction:isValid()
...
function AMOpenGuideAction:update()
    self.item:setJobDelta(self:getJobDelta());
end

function AMOpenGuideAction:start()
    self.character:getEmitter():playSoundImpl("pageflip", nil)
    self.item:setJobType(getText("ContextMenu_OpeningAMGuide"));
...
function AMOpenGuideAction:stop()
	ISBaseTimedAction.stop(self);
	self.character:setReading(false);
	self.item:setJobDelta(0.0);
end

function AMOpenGuideAction:perform()
	self.character:setReading(false);
    ISBaseTimedAction.perform(self);
	self.item:setJobDelta(0.0);
	AMGuideManager.onKeyPressed();
end

function AMOpenGuideAction:new(character, item, time)
	local o = {};
	setmetatable(o, self);
	self.__index = self;
