...
AMManualMenu.doMenu = function(player, context, items)
	...
	for i,v in ipairs(items) do
		...
		if tempitem:getType() == "ManualAM" then
			manual = tempitem;
		end
	end
	
	if manual then
		local option = context:addOption(getText("ContextMenu_OpenAMManual"), manual, AMManualMenu.OpenManual_OnCreate, player);
		local tooltip = ISWorldObjectContextMenu.addToolTip();
		tooltip:setName(getText("ContextMenu_OpenAMManual"));
		tooltip.description = tooltip.description .. getText("ContextMenu_OpenAMManualTT");
		tooltip:setTexture(manual:getTexture():getName());
		option.toolTip = tooltip;
	end
end

AMManualMenu.OpenManual_OnCreate = function(item, playernum)
	...
	ISTimedActionQueue.add(AMOpenGuideAction:new(player, item, nil))

end

Events.OnFillInventoryObjectContextMenu.Add(AMManualMenu.doMenu);